...
void TMDDelay::setSampleRate(double inSampleRate)
{
...
void TMDDelay::process(float* inAudio,
					   float inTime,
					   float inFeedback,
					   float inWetDry,
					   float inType,
					   ...
					   int inNumSamplesToRender)
{
	...
	const float feedbackMapped = 
		...
		((int)inType==kTMDDelayType_Delay) ? jmap(inFeedback, 0.0f, 1.0f, 0.0f, 1.20f) : 0.f;

	for (int i = 0; i < inNumSamplesToRender; i++) {
		
		if ((int)inType == kTMDDelayType_Delay) {
			//delay
...
double TMDDelay::getInterpolatedSample(float inDelayTimeInSamples)
{
