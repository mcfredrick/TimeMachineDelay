...
TMDVUMeter::TMDVUMeter(TimeMachineDelayAudioProcessor* inProcessor)
	:	mParameterID(-1),
		mCh0Level(0),
		mCh1Level(0),
...
TMDVUMeter::~TMDVUMeter()
{

}

void TMDVUMeter::paint(Graphics& g)
{
	const int meter_width = getParentWidth() / 5;

	//set meter background color
	g.setColour(TMDColour_6);
	//left channel meter
...
void TMDVUMeter::setParameterID(int inParameterID)
{
