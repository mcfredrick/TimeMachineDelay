...
TMDPanelBase::TMDPanelBase(TimeMachineDelayAudioProcessor* inProcessor)
	:	mProcessor(inProcessor)
{

}

TMDPanelBase::~TMDPanelBase()
{

}

void TMDPanelBase::mouseEnter(const MouseEvent& event)
{
	repaint();
}

void TMDPanelBase::mouseExit(const MouseEvent& event)
{
	repaint();
}

void TMDPanelBase::paint(Graphics& g)
{
		...
		switch (glow_type)
		{
			case(0): {
				...
				for (int i = 0; i < 4; i++) {
					...
					g.setColour(TMDColour_glow.withAlpha(1.0f - ((float)i * 0.2f)));
